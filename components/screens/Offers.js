...
function Offers({route, navigation}) {
  ...
  const {data, loading} = useSelector((state) => state.getAllSuggestionsSlice);
  const [citys, setCitys] = useState([]);
  const typeContainer = ["40 ST", "20 (30)", "20 (24)", "40 HQ"];
  const secondaryTabs = ["Поиск КТК", "Продажа КТК", "Выдача КТК", "Контейнерный сервис", "Заявка на ТЭО",];
  ...
  const favoriteList = useSelector((state) => state.getAllSuggestionsSlice.favoriteList);

  const dispatch = useDispatch();
  let liked = false;
  useEffect(() => {
    setId(activeSecondaryTab === "Поиск КТК" ? "2" : activeSecondaryTab === "Продажа КТК" ? "5" : activeSecondaryTab === "Выдача КТК" ? "3" : activeSecondaryTab === "Контейнерный сервис" ? "6" : "7");
  }, [activeSecondaryTab]);
  ...
  const filteretData = () => {
    ...
    dispatch(allSuggestionRequest({
      ...
      type_container: containerType === "40 ST" || containerType === "20 (30)" ? 4 : containerType === "20 (24)" ? 2 : containerType === "40 HQ" ? 3 : null,
    }))
    .unwrap()
    .then(() => setLoad(false));
  };
  ...
  const filterTypeContainer = (data) => {
    ...
    dispatch(allSuggestionRequest({
      ...
      to_city: cityToName ? cityToName.last_id : null,
      from_city: cityFromName ? cityFromName.last_id : null,
      type_container: data === "40 ST" || data === "20 (30)" ? 4 : data === "20 (24)" ? 2 : data === "40 HQ" ? 3 : null,
    }))
    .unwrap()
    .then(() => setLoad(false));
  };
  ...
  const resetFiltered = () => {
    ...
    setFromCityName(null);
    setToCityName(null);
    dispatch(allSuggestionRequest({token, id: activeSecondaryTab, offset: 5}));
  };
  ...
  return (<>
    {loading && (<Modal backdropOpacity={0.75} isVisible={true}>
      <View>
    ...
    >
      {searchValue ? (<View style={styles.searchModal}>
        <FlatList
          data={citys}
      ...
      <SwipeListView
        ...
        ListFooterComponent={() => {
          return data.length === 5 && activeTab !== "Избранное" ? (<View
            style={{
              flex: 1,
              height: 50,
              alignItems: "center",
        ...
        renderHiddenItem={({item}) => (<View style={styles.hiddenWrapper}>
          <TouchableOpacity
            style={styles.hiddenItem}
            onPress={() => navigation.navigate("SendOffer", {
              currentPage: "Отправить предложение", item, id,
            })}
          >
...
const styles = StyleSheet.create({
  searchRow: {
    ...
    height: 300, backgroundColor: "white", marginHorizontal: 20, borderRadius: 9, position: "absolute", // bottom: 0,
    top: 200, width: "90%", zIndex: 9999, flex: 1, padding: 16,
  }, pageCount: {
    width: 25,
    height: 25,
    ...
    flexDirection: "row", paddingBottom: 10, paddingHorizontal: WRAPPER_PADDINGS,
  }, item: {
    ...
    color: COLOR_8, fontSize: 10, fontFamily: "GothamProRegular", marginBottom: 6,
  }, ratingBlock: {
    flexDirection: "row", alignItems: "center",
  }, rating: {
    color: COLOR_9, fontSize: 10, fontFamily: "GothamProRegular", marginLeft: 5, marginRight: 15,
  }, companyName: {
    ...
    height: "100%", width: WRAPPER_PADDINGS, position: "absolute", zIndex: 2, top: 200, left: 0,
  }, fade: {
    width: "100%", height: "100%",
  }, date: {
    position: "absolute", bottom: -40, right: 0, fontFamily: "GothamProRegular",
  }, resetButton: {
    left: 25,
    bottom: -10,
    height: 30,
    ...
    fontSize: 12, color: "red", fontFamily: "GothamProRegular",
  }, empty: {
